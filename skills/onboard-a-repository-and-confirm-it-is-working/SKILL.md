...
**Setting `eligibilityLabel` creates the label on the repository**, so you no
longer have to pick one that is already there. The update stores the name and
then creates it, best-effort, once the write itself has landed — and only in
`labeled` mode, since in `all-issues` mode the label gates nothing.

**Read `eligibilityLabelWarning` on the response.** When the label could not be
created the update still succeeds, and that field names the label and the
repository so you can create it by hand. Its ABSENCE is the confirmation the
label is there — but only on a write that asserted eligibility, meaning one that
named `eligibilityLabel` or set `eligibilityMode` to `labeled`. A patch that
only pauses the repository or changes the order omits the field too, and there
it claims nothing, because nothing looked.

If you were warned, fix the cause and **send the same patch again**: re-asserting
the label is a no-op write that still creates it, so that is how you retry.
